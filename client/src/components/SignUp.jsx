...
function SignUp(props) {
  ...
  return (
    ...
    <input onChange={(e)=>setUserName(e.target.value)} className='border border-black p-2 rounded' placeholder='User Name'/>
    <input onChange={(e)=>setPassword(e.target.value)} className='border border-black p-2 rounded' placeholder='Password'/>
    <button onClick={handleSignUp} className='border border-green-700 text-green-700 rounded w-20 h-10 hover:bg-green-700 hover:text-white active:shadow-md active:bg-green-800'>Sign Up</button>
